...
class PCB_resnext(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training:
            part_fea = [self.classifier0.add_block(x[:, :, 0].squeeze()),
                        self.classifier1.add_block(x[:, :, 1].squeeze()),
                        self.classifier2.add_block(x[:, :, 2].squeeze()),
                        self.classifier3.add_block(x[:, :, 3].squeeze()),
                        self.classifier4.add_block(x[:, :, 4].squeeze()),
                        self.classifier5.add_block(x[:, :, 5].squeeze())]
