...
parser.add_argument('--test_dir', default='./test_set_B', type=str, help='./test_data')
...
def load_network2(network):
    # save_path = os.path.join('./model_gray2',name,'net_%s.pth'%opt.which_epoch)
    save_path = os.path.join('./152_82.64.pth')
    network.load_state_dict(torch.load(save_path))
    return network
def load_network3(network):
    # save_path = os.path.join('./model_gray2',name,'net_%s.pth'%opt.which_epoch)
    save_path = os.path.join('./resnext82.98.pth')
    network.load_state_dict(torch.load(save_path))
    return network

def load_network4(network):
    # save_path = os.path.join('./model_gray2',name,'net_%s.pth'%opt.which_epoch)
    save_path = os.path.join('./101_82.29.pth')
    network.load_state_dict(torch.load(save_path))
    return network
...
def cal_top_200_similar_ind(a1, a2,a3,a4,b1,b2,b3,b4):
    cos_mat = cal_cos_dis_2_matrix(a1, a2,a3,a4,b1,b2,b3,b4)
...
def load_data(path):
    ...
    return gal_fea1,gal_fea2, gal_fea3,gal_fea4,que_fea1, que_fea2,que_fea3,que_fea4,l_gal, l_que
...
def compute_result(path, save_path):
    gal_fea1, gal_fea2,gal_fea3,gal_fea4,que_fea1,que_fea2, que_fea3,que_fea4,l_gal, l_que = load_data(path)
    sim_ind = cal_top_200_similar_ind(que_fea1, que_fea2,que_fea3,que_fea4,gal_fea1,gal_fea2,gal_fea3,gal_fea4)
...
print('-------test-----------')
model_structure = PCB_ibn(class_num=4768)  # for duke
...
if torch.cuda.device_count() > 1:
    model3 = nn.DataParallel(model_structure)
model3 = load_network3(model3)

model_structure = PCB_101(class_num=4768)  # for duke
model4 = load_network4(model_structure)
if torch.cuda.device_count() > 1:
...
gallery_feature4 = extract_feature(model4, dataloaders['gallery'])
query_feature4= extract_feature(model4, dataloaders['query'])

# Save to Matlab for check
result = {'gallery_fea1': gallery_feature1.numpy(), 'gallery_fea2': gallery_feature2.numpy(),'gallery_fea3': gallery_feature3.numpy(),'gallery_fea4': gallery_feature4.numpy(),'gallery_path': g_path, 'query_fea1': query_feature1.numpy(),'query_fea2': query_feature2.numpy(),'query_fea3': query_feature3.numpy(),'query_fea4': query_feature4.numpy(),
          'query_path': q_path}
