...
from model.model_test import PCB
...
def load_network(network):
    ...
    save_path = os.path.join('/home/guoqiang/reid_competition/checkpoints/model2/net_59.pth')
...
#a is the query matrix, (m,fea_shape)
#b is the gallery matrix, (n,fea_shape)
#the output is a (m,n) cosine simalarity matrix
...
def get_top_200_similar(sim_ind,l_gal,l_que):
    ...
    for i in range(len(l_que)):
        l = []
        for j in range(sim_ind.shape[1]-1,-1,-1):
            l.append(osp.split(l_gal[sim_ind[i,j]])[-1])
        res_dict[osp.split(l_que[i])[-1]] = l
    return res_dict
